...
const SQL_SEARCH_PRODUCTS = (word: string, top: number, skip: number) =>
  `SELECT FIRST ${top} SKIP ${skip} E.PRODUTO, E.REFERENCIA, E.NOME, E.PRECO, E.QTDATUAL, E.VENDA, E.TRANCAR, E.ATIVO, E.APLICACOES, E.LOCAL, E.FABRICANTE, F.NOME AS FABRICANTE_NOME FROM EST E LEFT JOIN FAB F ON E.FABRICANTE = F.FABRICANTE WHERE ${SQL_SEARCH_PRODUCTS_FILTER(word)} ORDER BY E.PRODUTO`;
